...
export const DocumentCard: React.FC<DocumentCardProps> = ({ doc, onSelectDoc, onDiscussDoc }) => {
  ...
  return (
    <div
      ...
      className={`group relative bg-white dark:bg-[#121826] border border-slate-200 dark:border-slate-800/90 border-l-[3.5px] ${getSubjectSpineClass(
        doc.subject
    ...
    >
      <div>
        ...
        <div className="flex items-center justify-between gap-2 mb-3">
          <div className="flex items-center gap-1.5 flex-wrap">
            <span className="text-[10px] font-bold uppercase tracking-wider px-2.5 py-0.5 rounded-full border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-900 text-slate-800 dark:text-slate-200 flex items-center gap-1.5 shadow-2xs">
              <span
        ...
        {/* Title */}
        <h3 className="text-sm sm:text-base font-bold text-slate-900 dark:text-slate-100 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors line-clamp-2 leading-snug">
          {doc.title}
        </h3>
        ...
        <div className="flex flex-wrap items-center gap-1.5 mt-3">
          {doc.tags.slice(0, 3).map((tag) => (
            <span
              key={tag}
              className="text-[10px] bg-slate-50 dark:bg-slate-900/80 text-slate-600 dark:text-slate-400 px-2 py-0.5 rounded-lg border border-slate-200 dark:border-slate-800 font-mono"
            >
              #{tag}
            </span>
          ))}
      ...
      <div className="mt-5 pt-3.5 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between gap-2">
        <div className="text-[11px] text-slate-500 dark:text-slate-400 min-w-0">
          <div className="truncate font-medium text-slate-700 dark:text-slate-300">By {doc.uploaderName || 'Classmate'}</div>
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0" onClick={(e) => e.stopPropagation()}>
          {onDiscussDoc && (
            <button
              ...
              className="px-2.5 py-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs font-semibold flex items-center gap-1.5 transition active:scale-95 cursor-pointer border border-slate-200 dark:border-slate-700"
            >
          ...
          {doc.fileType === 'markdown' ? (
            ...
            <>
              ...
              <button
                ...
                title="Download PDF"
                className="p-1.5 rounded-xl bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 transition active:scale-95 cursor-pointer border border-slate-200 dark:border-slate-700"
              >
