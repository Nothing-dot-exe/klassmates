...
export const ChatWallpaperModal: React.FC<ChatWallpaperModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/60 backdrop-blur-sm animate-in fade-in">
      <div className="bg-white dark:bg-[#0E1424] border border-zinc-200 dark:border-[#1F2A44] rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl shadow-zinc-950/20 dark:shadow-black/70 flex flex-col max-h-[90dvh] transition-colors">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-200 dark:border-[#1F2A44] bg-white dark:bg-[#0E1424]">
            ...
            return (
              <button
                ...
                className={`group relative text-left p-3.5 rounded-2xl border transition-all cursor-pointer overflow-hidden ${
                  isSelected
                    ? 'border-indigo-500 bg-indigo-50/10 dark:bg-[#161F36] ring-2 ring-indigo-500/20'
                    : 'border-zinc-200 dark:border-[#1F2A44] hover:border-indigo-400 dark:hover:border-indigo-500/60 bg-white dark:bg-[#121A2D] shadow-xs'
                }`}
              >
                {/* Visual miniature preview box */}
                <div
                  style={bg.containerStyle}
                  className="h-20 w-full rounded-xl border border-zinc-200 dark:border-[#1F2A44] mb-2.5 shadow-inner relative overflow-hidden flex items-center justify-center"
                >
                  {isSelected && (
        ...
        {/* Footer */}
        <div className="px-5 py-3 border-t border-zinc-200 dark:border-[#1F2A44] bg-zinc-50 dark:bg-[#121A2D] flex items-center justify-between text-xs text-zinc-500 dark:text-zinc-400">
