  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="relative p-[2px] rounded-full border border-zinc-200 dark:border-card-border shadow-xs flex-shrink-0">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
            src={avatarUrl}
            alt={user.name}
            className="w-18 h-18 sm:w-20 sm:h-20 rounded-full object-cover bg-zinc-100 dark:bg-card-muted"
          />
          {isGif && (
        ...
        <div className="flex-1 grid grid-cols-3 text-center gap-1.5">
          <div className="p-2 rounded-xl bg-zinc-50 dark:bg-card-muted border border-zinc-200 dark:border-card-border">
          ...
          <div className="p-2 rounded-xl bg-zinc-50 dark:bg-card-muted border border-zinc-200 dark:border-card-border">
            ...
            <div className="text-[10px] text-zinc-500 dark:text-zinc-400 font-medium">Enrolled</div>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-2">
        <div>
          <h3 className="text-sm sm:text-base font-bold text-zinc-950 dark:text-white flex items-center gap-2 flex-wrap">
            {hasNickname ? (
              <>
                <span className="text-zinc-950 dark:text-white font-extrabold">{user.nickname?.trim()}</span>
                <span className="text-xs text-zinc-500 dark:text-zinc-400 font-normal">({user.name})</span>
              </>
            ) : (
              <span className="text-zinc-950 dark:text-white font-black">{user.name}</span>
            )}
          </h3>
        ...
        {!isEditing && (
          <div className="p-3 rounded-2xl bg-zinc-50 dark:bg-card-muted border border-zinc-200 dark:border-card-border text-xs text-zinc-800 dark:text-zinc-200 whitespace-pre-wrap leading-relaxed shadow-xs">
            {user.bio ? (
              user.bio
            ) : (
