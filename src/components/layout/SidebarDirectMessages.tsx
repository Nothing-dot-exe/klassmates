  ...
  return (
    <div className="space-y-1">
      <div className="text-[11px] font-bold text-slate-400 dark:text-slate-400 uppercase tracking-wider px-2 pb-1 flex items-center justify-between">
        ...
        <span className="font-mono text-xs font-semibold text-slate-400 dark:text-slate-400">#{otherStudents.length}</span>
      </div>

      {otherStudents.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-slate-300 dark:border-slate-800 bg-white/70 dark:bg-[#0E1528]/60 p-4 text-center flex flex-col items-center justify-center">
          <div className="w-9 h-9 rounded-full bg-slate-100 dark:bg-slate-800/70 flex items-center justify-center text-slate-500 dark:text-slate-400 mb-2">
          ...
          <h4 className="text-xs font-bold text-slate-900 dark:text-slate-200 mb-1">No students added yet</h4>
          <p className="text-[11px] text-slate-500 dark:text-slate-400 max-w-[200px] leading-relaxed mb-2.5">
            Add via Admin Panel or share Class Code with your batchmates.
          </p>
          <button
            onClick={handleCopyCode}
            type="button"
            className="px-3 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 active:scale-95 text-indigo-600 dark:text-indigo-300 text-[11px] font-semibold tracking-wide border border-slate-200 dark:border-slate-700 transition-all flex items-center gap-1.5 cursor-pointer shadow-2xs"
          >
            ...
            >
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                >
                  ...
                  />
                  <span
                    className={`absolute -bottom-0.5 -right-0.5 w-2 h-2 rounded-full ring-2 ring-white dark:ring-[#080C15] ${
                      st.status === 'studying'
                        ? 'bg-amber-400'
                        : st.status === 'online'
                          ? 'bg-emerald-500'
