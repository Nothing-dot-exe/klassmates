...
export const AdminDocumentsCard: React.FC<AdminDocumentsCardProps> = ({
  ...
  return (
    <div className="bg-card border border-zinc-200 dark:border-card-border rounded-3xl p-4 sm:p-8 space-y-5 shadow-sm">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-zinc-200 dark:border-card-border">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-black text-zinc-950 dark:text-white flex items-center gap-2">
              ...
              <span className="text-[10px] px-2.5 py-0.5 rounded-full bg-indigo-50 dark:bg-[#24302c] text-indigo-600 dark:text-indigo-400 font-bold border border-indigo-200 dark:border-indigo-500/30">
      ...
      {documents.length === 0 ? (
        <div className="text-center py-8 border border-dashed border-zinc-200 dark:border-card-border rounded-2xl p-4 text-xs text-zinc-600 dark:text-zinc-400 font-medium bg-zinc-50/50 dark:bg-background/40">
        ...
        <div className="divide-y divide-zinc-200 dark:divide-[#27272a] max-h-80 overflow-y-auto no-scrollbar">
          {filtered.map((doc) => {
            const isConfirming = confirmDeleteId === doc.id;
            return (
              <div key={doc.id} className="py-3 flex items-center justify-between gap-3 group">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="p-2 rounded-xl border border-zinc-200 dark:border-card-border bg-zinc-100 dark:bg-card-muted text-zinc-900 dark:text-indigo-400 flex-shrink-0">
                  ...
                  {isConfirming ? (
                    <div className="flex items-center gap-1.5 animate-in fade-in">
                      ...
                      <button
                        ...
                        className="px-2 py-1 rounded-lg bg-zinc-100 dark:bg-[#24302c] text-zinc-800 dark:text-white hover:bg-zinc-200 dark:hover:bg-[#27272a] text-[11px] font-semibold transition cursor-pointer border border-zinc-200 dark:border-card-border"
                      >
                        Cancel
                      </button>
                    </div>
                    ...
                    <button
                      ...
                      className="p-2 rounded-xl bg-zinc-100 dark:bg-card-muted hover:bg-rose-50 dark:hover:bg-rose-950/30 text-zinc-600 dark:text-zinc-400 hover:text-rose-600 dark:hover:text-rose-400 border border-zinc-200 dark:border-card-border hover:border-rose-300 transition active:scale-95 cursor-pointer"
